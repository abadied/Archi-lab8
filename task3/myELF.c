...
void linkcheck() {
    ...
    if (secsymtable != 0) {
        Elf32_Shdr *strsec = (Elf32_Shdr*)((char*)map_sheaders + ((secsymtable->sh_link) * shent_size));

        int tabsize = (secsymtable->sh_size) / 16;
        void* map_tab = (void*)((char*)map_start + secsymtable->sh_offset);
        char* symstrtab = (char*)map_start + strsec->sh_offset;
        ...
        for (j = 0; j < tabsize; j++) {
            symtab_ent = (Elf32_Sym*)((char*)map_tab + 16*j);
            if (strcmp((symstrtab + symtab_ent->st_name), "_start") == 0) {
...
void relocTable() {
    ...
    for (i = 0; i < shnum; i++) {
        ...
        if (sheader->sh_type == SHT_REL){
            int reltabsize = (sheader->sh_size) / 8;
            void* map_rel = (void*)((char*)map_start + sheader->sh_offset);
            Elf32_Rel *rel_entry;
            int j;
            for (j = 0; j < reltabsize; j++) {
                rel_entry = (Elf32_Rel*)((char*)map_rel + 8*j);
                printf("%08x %08x\n", rel_entry->r_offset, rel_entry->r_info);
            }
            printf("\n");
        }
        else if (sheader->sh_type == SHT_RELA){
            int relatabsize = (sheader->sh_size) / 12;
            void* map_rela = (void*)((char*)map_start + sheader->sh_offset);
            Elf32_Rela *rela_entry;
            int j;
            for (j = 0; j < relatabsize; j++) {
                rela_entry = (Elf32_Rela*)((char*)map_rela + 12*j);
                printf("%08x %08x %08x\n", rela_entry->r_offset, rela_entry->r_info, rela_entry->r_addend);
            }
            printf("\n");
        }
    }
...
void relocTableSem() {
    ...
    for (i = 0; i < shnum; i++) {
        sheader = (Elf32_Shdr*)((char*)map_sheaders + shent_size*i);
        if (sheader->sh_type == SHT_REL){
            Elf32_Shdr *symtable_secheader = (Elf32_Shdr*)((char*)map_sheaders + sheader->sh_link*shent_size);
            Elf32_Shdr *strsec = (Elf32_Shdr*)((char*)map_sheaders + ((symtable_secheader->sh_link) * shent_size));

            void* map_tab = (void*)((char*)map_start + symtable_secheader->sh_offset);
            char* symstrtab = (char*)map_start + strsec->sh_offset;
            ...
            for (j = 0; j < reltabsize; j++) {
                rel_entry = (Elf32_Rel*)((char*)map_rel + 8*j);
                printf("%08x\t", rel_entry->r_offset);
                printf("%08x\t", rel_entry->r_info);

                Elf32_Sym *syment = (Elf32_Sym*)((char*)map_tab + 16*ELF32_R_SYM(rel_entry->r_info));

                printf("%d\t", ELF32_R_TYPE(rel_entry->r_info));
                printf("%s\t", symstrtab + syment->st_name);
                printf("\n");
            }
            printf("\n");
        }
            ...
            for (j = 0; j < relatabsize; j++) {
                rela_entry = (Elf32_Rela*)((char*)map_rela + 12*j);
                printf("%08x\t", rela_entry->r_offset);
                printf("%08x\t", rela_entry->r_info);
                printf("\n");
            }
            printf("\n");
        }
...
int main(int argc, char** argv){
    ...
    struct func_desc menu[] = {	{"0-Toggle Debug Mode", toggleDebugMode},
                                ...
                                {"5-Relocation Tables - Raw",relocTable},
                                {"6-Relocation Tables - Semantic",relocTableSem},
    							{"7-Quit", quit},
    							{NULL,NULL}};
    while(1){
    	if(debug_mode){
